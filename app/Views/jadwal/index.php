    ...
    <div class="modal modal-md">
        ...
        <form action="<?= base_url('jadwal/store') ?>" method="POST">
            ...
            <div class="modal-body">
                <div class="form-row full">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-input" name="nama_siswa" placeholder="Nama lengkap siswa" required>
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-group">
                ...
                <div class="form-row full">
                    <div class="form-group">
                        ...
                        <textarea class="form-textarea" name="topik" placeholder="Uraikan topik atau permasalahan yang akan dibahas..." required></textarea>
                    </div>
                </div>
                <div class="form-row full">
                    <div class="form-group">
            ...
            <div class="modal-footer">
                ...
                <button type="submit" class="btn-primary"><i class="fa fa-save"></i> Simpan Jadwal</button>
            </div>
        </form>
    </div>
</div>
...
<div class="modal-overlay" id="modalHapus">
    <div class="modal modal-sm">
        ...
        <div class="modal-body" style="text-align:center;padding:32px 26px">
            ...
            <p class="confirm-msg">
                Anda akan menghapus jadwal konseling milik<br>
...
<aside class="sidebar" id="sidebar">
    ...
    <div class="sidebar-section">
        ...
        <a class="nav-item <?= str_starts_with(uri_string(),'jadwal')?'active':'' ?>" href="<?= base_url('jadwal') ?>">
            <i class="fa fa-calendar-check"></i> Jadwal Konseling <span class="nav-badge warn"><?= $stats_jadwal['terjadwal'] ?? 0 ?></span>
        </a>
    ...
    <div class="page-content">
        ...
        <div class="page-header">
            ...
            <div class="page-header-right">
                ...
                <button class="btn-primary" onclick="openModal('modalTambah')"><i class="fa fa-plus"></i> Tambah Jadwal</button>
            </div>
        </div>
        ...
        <div class="card" style="margin-bottom:28px">
            <div class="tab-nav">
                <a class="tab-btn active" href="#">
                    ...
                    <span class="tab-count"><?= count($jadwal ?? []) ?></span>
                </a>
            </div>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td colspan="7" style="text-align:center;padding:40px;color:var(--gray-400)">
                                <i class="fa fa-calendar-xmark" style="font-size:28px;display:block;margin-bottom:10px;color:var(--gray-200)"></i>
                        ...
                        foreach ($jadwal as $i => $j):
                            ...
                            $statusClass = strtolower($j['status'] ?? 'terjadwal');
                            $jsNama  = addslashes($nama);
                            $jsTopik = addslashes($j['topik'] ?? '');
                        ?>
                        <tr>
                            ...
                            <td style="color:var(--gray-600)"><?= date('d M Y', strtotime($j['tanggal'])) ?></td>
                            <td style="color:var(--gray-600)"><?= esc(substr($j['waktu'], 0, 5)) ?> WIB</td>
                            <td style="max-width:220px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap"><?= esc($j['topik']) ?></td>
                            <td><span class="badge <?= $statusClass ?>"><?= esc($j['status']) ?></span></td>
                            <td>
                                <div class="action-btns">
                                    <button class="btn-icon view" title="Detail"
                                        onclick="openDetail('<?= $jsNama ?>','<?= $j['tanggal'] ?>','<?= substr($j['waktu'],0,5) ?>','<?= $statusClass ?>','<?= esc($j['status']) ?>',`<?= $jsTopik ?>`)">
                                        <i class="fa fa-eye"></i>
                                    </button>
                                    <button class="btn-icon del" title="Hapus"
                                        onclick="openHapus(<?= $j['id'] ?>,'<?= $jsNama ?>')">
            ...
            <div class="pagination-bar">
                <span>Menampilkan <?= count($jadwal ?? []) ?> data</span>
...
function openDetail(nama,tgl,waktu,statusClass,statusLabel,topik){
    ...
    document.getElementById('detailAvatar').style.background=colors[hash%colors.length];
    document.getElementById('detailNama').textContent=nama;
    document.getElementById('detailMeta').textContent=fmtTgl(tgl)+' • '+waktu+' WIB';
    document.getElementById('dTanggal').textContent=fmtTgl(tgl);
    document.getElementById('dWaktu').textContent=waktu+' WIB';
    document.getElementById('dStatus').innerHTML=`<span class="badge ${statusClass}">${statusLabel}</span>`;
    document.getElementById('dCreated').textContent='—';
    document.getElementById('dTopik').textContent=topik||'—';
    openModal('modalDetail');
}

function openHapus(id,nama){
    document.getElementById('hapusNama').textContent=nama;
    document.getElementById('hapusLink').href=`<?= base_url('jadwal/hapus/') ?>${id}`;
