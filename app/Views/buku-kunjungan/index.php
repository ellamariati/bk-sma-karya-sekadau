...
<head>
    ...
    <style>
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .stat-card.blue{border-left-color:var(--blue-500)}.stat-card.yellow{border-left-color:var(--warning)}.stat-card.green{border-left-color:var(--success)}.stat-card.purple{border-left-color:var(--purple)}.stat-card.teal{border-left-color:var(--teal)}
        ...
        .badge.proses{background:#fef3c7;color:#92400e}.badge.selesai{background:#d1fae5;color:#065f46}
        .badge.mandiri{background:#dbeafe;color:#1d4ed8}.badge.panggilan{background:#ede9fe;color:#5b21b6}
        .badge.bk{background:var(--blue-100);color:var(--blue-700)}.badge.wk{background:#ede9fe;color:#5b21b6}.badge.bk-wk{background:#fce7f3;color:#9d174d}
        ...
        .jb-pill.pribadi{background:#dbeafe;color:#1d4ed8}.jb-pill.sosial{background:#fce7f3;color:#9d174d}.jb-pill.belajar{background:#d1fae5;color:#065f46}.jb-pill.karir{background:#ede9fe;color:#5b21b6}
        ...
        .btn-icon.view{background:var(--blue-100);color:var(--blue-600)}.btn-icon.edit{background:#fef3c7;color:#b45309}.btn-icon.del{background:#fee2e2;color:#dc2626}.btn-icon.check{background:#d1fae5;color:#059669}
        ...
        @media(max-width:768px){:root{--sidebar-w:0px}.sidebar{transform:translateX(-270px);width:270px}.sidebar.open{transform:translateX(0)}.main-wrapper{margin-left:0}.navbar-hamburger{display:flex}.page-content{padding:20px 18px}.form-row{grid-template-columns:1fr}}
    </style>
</head>
<body>
...
<div class="main-wrapper">
    ...
    <div class="page-content">
        ...
        <div class="stats-grid">
            <div class="stat-card blue" style="animation-delay:.05s"><div class="stat-ico"><i class="fa fa-book-open"></i></div><div><div class="stat-num"><?= $stats['total'] ?? 0 ?></div><div class="stat-lbl">Total Kunjungan</div></div></div>
            <div class="stat-card yellow" style="animation-delay:.08s"><div class="stat-ico"><i class="fa fa-spinner"></i></div><div><div class="stat-num"><?= $stats['proses'] ?? 0 ?></div><div class="stat-lbl">Sedang Diproses</div></div></div>
            <div class="stat-card green" style="animation-delay:.11s"><div class="stat-ico"><i class="fa fa-circle-check"></i></div><div><div class="stat-num"><?= $stats['selesai'] ?? 0 ?></div><div class="stat-lbl">Selesai</div></div></div>
            <div class="stat-card teal" style="animation-delay:.14s"><div class="stat-ico"><i class="fa fa-person-walking"></i></div><div><div class="stat-num"><?= $stats['mandiri'] ?? 0 ?></div><div class="stat-lbl">Mandiri</div></div></div>
            <div class="stat-card purple" style="animation-delay:.17s"><div class="stat-ico"><i class="fa fa-bullhorn"></i></div><div><div class="stat-num"><?= $stats['panggilan'] ?? 0 ?></div><div class="stat-lbl">Panggilan</div></div></div>
        </div>

        <div class="filter-bar">
            <div class="filter-search"><i class="fa fa-magnifying-glass"></i><input type="text" id="searchInput" placeholder="Cari nama siswa, keperluan..." oninput="filterData()"></div>
            <select class="filter-select" id="filterStatus" onchange="filterData()"><option value="">Semua Status</option><option value="proses">Proses</option><option value="selesai">Selesai</option></select>
            <select class="filter-select" id="filterJenisKunjungan" onchange="filterData()"><option value="">Semua Jenis</option><option value="mandiri">Mandiri</option><option value="panggilan">Panggilan</option></select>
            <select class="filter-select" id="filterJb" onchange="filterData()"><option value="">Semua Bimbingan</option><option value="pribadi">Pribadi</option><option value="sosial">Sosial</option><option value="belajar">Belajar</option><option value="karir">Karir</option></select>
            <select class="filter-select" id="filterKelas" onchange="filterData()"><option value="">Semua Kelas</option><option value="X">Kelas X</option><option value="XI">Kelas XI</option><option value="XII">Kelas XII</option></select>
        </div>
        ...
        <div class="table-card">
            <div class="table-wrap">
                <table>
                    <thead>
                        <tr>
                            <th>#</th><th>Hari / Tanggal</th><th>Nama Siswa</th><th>Jenis</th>
                            <th>Jenis Bimbingan</th><th>Keperluan</th><th>Hasil Kunjungan</th>
                            <th>TTD</th><th>Yang Menangani</th><th>Status</th><th>Aksi</th>
                        </tr>
                    </thead>
                    <tbody id="tableBody">
                        <?php if (empty($list_kunjungan)): ?>
                            <tr><td colspan="11"><div class="empty-state"><i class="fa fa-book-open"></i><p>Belum ada data kunjungan</p></div></td></tr>
                        <?php else: ?>
                            <?php
                            ...
                            <?php
                                $tgl=$kj['tanggal']; $hariStr=$hariId[date('l',strtotime($tgl))]??date('l',strtotime($tgl)); $tglStr=date('d/m/Y',strtotime($tgl));
                            ...
                            <tr class="tr-data"
                                ...
                                <td style="white-space:nowrap">
                                    ...
                                    <?php if($jamStr!=='—'): ?><div style="font-size:11px;color:var(--gray-400)"><i class="fa fa-clock" style="font-size:10px"></i> <?= $jamStr ?></div><?php endif; ?>
                                </td>
                                <td>
                                    <div class="td-name"><?= esc($nama) ?></div>
                                    <div class="td-sub"><?= esc($kj['kelas']??'—') ?> · <?= esc($kj['nisn']??'—') ?></div>
                                </td>
                                <td><span class="badge <?= $kj['jenis_kunjungan'] ?>"><?= $kj['jenis_kunjungan']==='mandiri'?'🚶 Mandiri':'📢 Panggilan' ?></span></td>
                                <td>
                                    <?php if(!empty($jbArr)): ?>
                                ...
                                <td>
                                    <div class="action-btns">
                                        ...
                                        <button class="btn-icon check" title="<?= $kj['status']==='proses'?'Tandai Selesai':'Tandai Proses' ?>" onclick="toggleStatus(<?= $kj['id'] ?>,'<?= $kj['status'] ?>')"><i class="fa fa-<?= $kj['status']==='proses'?'check':'rotate-left' ?>"></i></button>
            ...
            <div class="info-bar" id="infoBar">Menampilkan <?= count($list_kunjungan) ?> data kunjungan</div>
        </div>
    </div>
</div>
...
function getAvatarColor(nama){let h=0;for(let c of nama)h=(h+c.charCodeAt(0))%AVATAR_COLORS.length;return AVATAR_COLORS[h];}
function getInitials(nama){return nama.trim().split(' ').map(w=>w[0]).join('').toUpperCase().slice(0,2);}
...
function renderSiswaList(prefix, query=''){
    ...
    const filtered=query
        ...
        :SISWA_DATA;
    if(!filtered.length){list.innerHTML='<div class="siswa-no-result"><i class="fa fa-user-slash" style="font-size:22px;display:block;margin-bottom:6px;color:var(--gray-200)"></i>Siswa tidak ditemukan</div>';return;}
...
function closeAllSiswaDropdowns(){document.querySelectorAll('.siswa-dropdown').forEach(d=>d.classList.remove('show'));}
function filterSiswaOptions(prefix){renderSiswaList(prefix,document.getElementById(prefix+'_siswa_search').value);}
...
document.addEventListener('click',function(e){if(!e.target.closest('.siswa-search-wrap'))closeAllSiswaDropdowns();});
...
function closeModal(id){document.getElementById(id).classList.remove('show');closeAllSiswaDropdowns();}
document.querySelectorAll('.modal-overlay').forEach(el=>{el.addEventListener('click',function(e){if(e.target===this){this.classList.remove('show');closeAllSiswaDropdowns();}})});

function appendHasil(text){const ta=document.getElementById('inputHasil');ta.value=ta.value?ta.value+'\n- '+text:'- '+text;ta.focus();}
...
function filterData(){
    ...
    document.querySelectorAll('.tr-data').forEach(tr=>{
        const show=(!q||tr.dataset.nama.includes(q)||tr.dataset.keperluan.includes(q))
                 &&(!st||tr.dataset.status===st)&&(!jk||tr.dataset.jenis===jk)
...
function openDetail(id){
    ...
    fetch(BASE_URL+'buku-kunjungan/detail/'+id).then(r=>r.json()).then(res=>{
        ...
        const jkBg=d.jenis_kunjungan==='mandiri'?'#dbeafe':'#ede9fe',jkCol=d.jenis_kunjungan==='mandiri'?'#1d4ed8':'#5b21b6';
        ...
        document.getElementById('detailBody').innerHTML=`
            ...
            <div class="detail-section"><div class="detail-section-title">Jenis Kunjungan &amp; Bimbingan</div>
                <div style="display:flex;flex-wrap:wrap;gap:8px;padding:4px 0">
                    <span style="display:inline-flex;padding:4px 12px;border-radius:20px;font-size:12px;font-weight:700;background:${jkBg};color:${jkCol}">${d.jenis_kunjungan==='mandiri'?'🚶 Mandiri':'📢 Panggilan'}</span>
                    ${jbHtml}
                </div>
            </div>
            <div class="detail-section"><div class="detail-section-title">Keperluan / Masalah</div><div class="detail-text-box">${(d.keperluan||'').replace(/\n/g,'<br>')}</div></div>
            <div class="detail-section"><div class="detail-section-title">Hasil / Tindak Lanjut Kunjungan</div><div class="detail-text-box">${(d.hasil_kunjungan||'').replace(/\n/g,'<br>')}</div></div>
            <div class="detail-section"><div class="detail-section-title">Penanganan &amp; Status</div>
                <div class="detail-grid">
                    ...
                    <div class="detail-item"><span class="label">Status</span><span style="display:inline-flex;padding:4px 12px;border-radius:20px;font-size:12px;font-weight:700;background:${stBg};color:${stCol}">${d.status==='selesai'?'✓ Selesai':'⏳ Proses'}</span></div>
                </div>
            </div>`;
    });
}
...
function openEdit(id){
    fetch(BASE_URL+'buku-kunjungan/edit/'+id).then(r=>r.json()).then(res=>{
...
function toggleStatus(id,currentStatus){
    ...
    fetch(BASE_URL+'buku-kunjungan/status/'+id,{method:'POST',body:fd}).then(r=>r.json()).then(res=>{if(res.success)location.reload();});
}
