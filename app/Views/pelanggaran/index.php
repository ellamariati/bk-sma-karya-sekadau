...
<head>
    ...
    <style>
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .btn-sm-act{padding:6px 14px;border-radius:8px;border:1.5px solid var(--gray-200);background:white;font-family:'DM Sans',sans-serif;font-size:12px;font-weight:500;color:var(--gray-600);cursor:pointer;transition:var(--transition)}
        .btn-sm-act:hover,.btn-sm-act.active{background:var(--blue-500);border-color:var(--blue-500);color:white}
...
<?php
// Data dummy siswa untuk dropdown
$listSiswa = [
    ['id'=>1,'nisn'=>'0041230001','nama'=>'Aldi Firmansyah',   'kelas'=>'XI RPL 2'],
    ['id'=>2,'nisn'=>'0041230002','nama'=>'Putri Ayu Lestari', 'kelas'=>'X TKJ 1'],
    ['id'=>3,'nisn'=>'0041230003','nama'=>'Rafi Hidayat',      'kelas'=>'XII MM'],
    ['id'=>4,'nisn'=>'0041230004','nama'=>'Sinta Wulandari',   'kelas'=>'X AKL'],
    ['id'=>5,'nisn'=>'0041230005','nama'=>'Bima Saputra',      'kelas'=>'XI TSM 3'],
    ['id'=>6,'nisn'=>'0041230006','nama'=>'Nadia Pratiwi',     'kelas'=>'XII AP 1'],
    ['id'=>7,'nisn'=>'0041230007','nama'=>'Dandi Firmawan',    'kelas'=>'XI TKJ 2'],
    ['id'=>8,'nisn'=>'0041230008','nama'=>'Mega Rahayu',       'kelas'=>'X MM 1'],
    ['id'=>9,'nisn'=>'0041230009','nama'=>'Kevin Prasetyo',    'kelas'=>'XII RPL 1'],
    ['id'=>10,'nisn'=>'0041230010','nama'=>'Ayu Cahyani',      'kelas'=>'XI AKL 2'],
];
            ...
            <div class="modal-body">
                ...
                <div class="form-row full">
                    <div class="form-group">
                        ...
                        <select class="form-select" name="siswa_id" required onchange="fillSiswaInfo(this,'tambah')">
                            <option value="">— Pilih Siswa —</option>
                            <?php foreach ($listSiswa as $s): ?>
                            <option value="<?= $s['id'] ?>" data-kelas="<?= $s['kelas'] ?>" data-nisn="<?= $s['nisn'] ?>">
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="date" class="form-input" name="tanggal_kejadian"
                               value="<?= date('Y-m-d') ?>" required>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Lokasi Kejadian</label>
                        <input type="text" class="form-input" name="lokasi" placeholder="Contoh: Kantin, Kelas XI...">
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <select class="form-select" name="jenis_pelanggaran" id="selectJenisTambah"
                                required onchange="autoFillPoin('tambah')">
                            <option value="">— Pilih Jenis —</option>
                            <?php foreach ($jenisPelanggaran as $kat => $items): ?>
                            <optgroup label="Pelanggaran <?= $kat ?>">
                                <?php foreach ($items as $nama => $poin): ?>
                                <option value="<?= esc($nama) ?>" data-poin="<?= $poin ?>"
                ...
                <div class="form-row" style="margin-top:14px">
                    <div class="form-group">
                        ...
                        <input type="number" class="form-input" name="poin" id="inputPoinTambah"
...
<div class="modal-overlay" id="modalDetail">
    <div class="modal modal-md">
        ...
        <div class="modal-footer">
            ...
            <button class="btn-primary" onclick="openUpdateStatusFromDetail()"><i class="fa fa-refresh"></i> Update Status</button>
        </div>
    </div>
</div>
        ...
        <form id="formEdit" action="" method="POST">
            ...
            <div class="modal-body">
                ...
                <div class="form-row">
                    <div class="form-group">
                        <label class="form-label">Jenis Pelanggaran <span>*</span></label>
                        <select class="form-select" name="jenis_pelanggaran" id="selectJenisEdit"
                                required onchange="autoFillPoin('edit')">
                            <option value="">— Pilih Jenis —</option>
                            <?php foreach ($jenisPelanggaran as $kat => $items): ?>
                            <optgroup label="Pelanggaran <?= $kat ?>">
                                <?php foreach ($items as $nama => $poin): ?>
                ...
                <div class="form-row">
                    <div class="form-group">
                        <label class="form-label">Poin <span>*</span></label>
                        <input type="number" class="form-input" name="poin" id="inputPoinEdit"
        ...
        <form id="formStatus" action="" method="POST">
            ...
            <div class="modal-body">
                <p style="font-size:13px;color:var(--gray-500);margin-bottom:16px">
                    Perbarui status penanganan pelanggaran untuk:
                    <strong id="statusSiswaName" style="color:var(--gray-800)">—</strong>
                </p>
                <div class="status-options">
                    <label class="status-option" onclick="selectStatus(this,'baru')">
                        ...
                        <div class="status-option-text">
                            <div class="st-title">Baru Masuk</div>
                            <div class="st-desc">Pelanggaran baru diterima, belum ditangani</div>
                        </div>
                    </label>
                    <label class="status-option" onclick="selectStatus(this,'proses')">
                        ...
                        <div class="status-option-text">
                            <div class="st-title">Dalam Proses</div>
                            <div class="st-desc">Sedang dalam proses konseling / investigasi</div>
                        </div>
                    </label>
                    <label class="status-option" onclick="selectStatus(this,'diproses')">
                        ...
                        <div class="status-option-text">
                            <div class="st-title">Sudah Diproses</div>
                            <div class="st-desc">Pelanggaran telah diselesaikan dan ditutup</div>
                        </div>
                    </label>
                </div>
            </div>
...
<div class="main-wrapper">
    ...
    <div class="page-content">
        ...
        <div class="flash-alert error">
            <i class="fa fa-circle-exclamation"></i> <?= session()->getFlashdata('error') ?>
        </div>
        <?php endif; ?>
        ...
        <div class="tab-nav">
            <?php
            // Buat query string filter yang ada
            ...
            $qStr = $qStr ? '&'.$qStr : '';
            ?>
            <a class="tab-btn <?= $tab_aktif === 'baru'     ? 'active' : '' ?>"
            href="<?= base_url('pelanggaran?tab=baru'.$qStr) ?>">
                <i class="fa fa-circle-exclamation"></i> Baru Masuk
                <span class="tab-count red"><?= $stats['baru'] ?></span>
            </a>
            <a class="tab-btn <?= $tab_aktif === 'proses'   ? 'active' : '' ?>"
            href="<?= base_url('pelanggaran?tab=proses'.$qStr) ?>">
                <i class="fa fa-spinner"></i> Dalam Proses
                <span class="tab-count warn"><?= $stats['proses'] ?></span>
            </a>
            <a class="tab-btn <?= $tab_aktif === 'diproses' ? 'active' : '' ?>"
            href="<?= base_url('pelanggaran?tab=diproses'.$qStr) ?>">
                <i class="fa fa-circle-check"></i> Sudah Diproses
                <span class="tab-count"><?= $stats['diproses'] ?></span>
            </a>
            <a class="tab-btn <?= $tab_aktif === 'semua'    ? 'active' : '' ?>"
            href="<?= base_url('pelanggaran?tab=semua'.$qStr) ?>">
                ...
                <td>
                    <div class="action-btns">
                        <button class="btn-icon view" title="Detail"
                            onclick="openDetail(<?= $r['id'] ?>, '<?= esc($nama) ?>', '<?= esc($r['kelas'] ?? '') ?>', '<?= esc($r['jenis_pelanggaran']) ?>', '<?= $katClass ?>', <?= $r['poin'] ?>, '<?= $r['tanggal_kejadian'] ?>', '<?= $r['status'] ?>', '<?= (int)$r['notif_ortu'] ?>', '<?= esc($r['lokasi'] ?? '') ?>', `<?= addslashes($r['deskripsi'] ?? '') ?>`)">
                            <i class="fa fa-eye"></i>
                        </button>
                        <button class="btn-icon edit" title="Edit"
                            onclick="openEdit(<?= $r['id'] ?>, '<?= esc($nama) ?>', '<?= esc($r['kelas'] ?? '') ?>', '<?= esc($r['jenis_pelanggaran']) ?>', '<?= $katClass ?>', <?= $r['poin'] ?>, '<?= $r['tanggal_kejadian'] ?>', '<?= $r['status'] ?>', <?= (int)$r['notif_ortu'] ?>, '<?= esc($r['lokasi'] ?? '') ?>', `<?= addslashes($r['deskripsi'] ?? '') ?>`, <?= $r['siswa_id'] ?>)">
                            <i class="fa fa-pen"></i>
                        </button>
                        <button class="btn-icon ok" title="Update Status"
                            onclick="openUpdateStatus(<?= $r['id'] ?>, '<?= $r['status'] ?>', '<?= esc($nama) ?>')">
                            <i class="fa fa-rotate"></i>
                        </button>
                        <button class="btn-icon del" title="Hapus"
                            onclick="openHapus(<?= $r['id'] ?>, '<?= esc($nama) ?>', '<?= esc($r['jenis_pelanggaran']) ?>')">
...
// ══ AUTO FILL SISWA INFO ══
function fillSiswaInfo(sel, mode){
    const opt = sel.options[sel.selectedIndex];
    if(!opt.value){
        document.getElementById('siswaInfoTambah').style.display='none';
        return;
    }
    document.getElementById('kelasInfoTambah').value = opt.dataset.kelas || '';
...
// ══ TABLE DATA ══
const avatarColors=['#1a56db','#ef4444','#f59e0b','#10b981','#8b5cf6','#ec4899','#06b6d4','#f97316'];
const allData=[
    {id:1,inisial:'AF',color:'#ef4444',nama:'Aldi Firmansyah',  kelas:'XI RPL 2',jenis:'Perkelahian',          kat:'berat', poin:75, tgl:'2026-03-12',guru:'Bu Rina', status:'baru',   lokasi:'Kantin',  deskripsi:'Berkelahi dengan siswa lain di area kantin pada jam istirahat.',notif:0},
    {id:2,inisial:'PA',color:'#f59e0b',nama:'Putri Ayu Lestari',kelas:'X TKJ 1', jenis:'Membolos Pelajaran',  kat:'sedang',poin:30, tgl:'2026-03-12',guru:'Pak Budi',status:'baru',   lokasi:'—',       deskripsi:'Tidak hadir tanpa keterangan selama 3 hari berturut-turut.',notif:0},
    {id:3,inisial:'RH',color:'#8b5cf6',nama:'Rafi Hidayat',     kelas:'XII MM',  jenis:'Membawa Rokok',        kat:'berat', poin:75, tgl:'2026-03-12',guru:'Bu Rina', status:'baru',   lokasi:'Toilet',  deskripsi:'Kedapatan membawa dan merokok di toilet sekolah.',notif:0},
    {id:4,inisial:'SW',color:'#1a56db',nama:'Sinta Wulandari',  kelas:'X AKL',   jenis:'Terlambat Masuk',     kat:'ringan',poin:10, tgl:'2026-03-12',guru:'Pak Budi',status:'baru',   lokasi:'Gerbang', deskripsi:'Terlambat masuk 45 menit tanpa alasan jelas.',notif:0},
    {id:5,inisial:'BS',color:'#f97316',nama:'Bima Saputra',     kelas:'XI TSM 3',jenis:'Seragam Tidak Sesuai', kat:'ringan',poin:10, tgl:'2026-03-12',guru:'Bu Siti', status:'baru',   lokasi:'Kelas',   deskripsi:'Tidak memakai dasi dan sepatu tidak hitam.',notif:0},
    {id:6,inisial:'NP',color:'#06b6d4',nama:'Nadia Pratiwi',   kelas:'XII AP 1',jenis:'Tidak Hadir (Alpha)',  kat:'ringan',poin:10, tgl:'2026-03-12',guru:'Bu Rina', status:'baru',   lokasi:'—',       deskripsi:'Tidak hadir tanpa surat keterangan.',notif:0},
    {id:7,inisial:'DF',color:'#f59e0b',nama:'Dandi Firmawan',   kelas:'XI TKJ 2',jenis:'Penggunaan HP di Kelas',kat:'sedang',poin:25,tgl:'2026-03-11',guru:'Pak Budi',status:'baru',   lokasi:'Kelas XI TKJ 2',deskripsi:'Bermain game di HP saat pelajaran berlangsung.',notif:0},
    {id:8,inisial:'MR',color:'#10b981',nama:'Mega Rahayu',      kelas:'X MM 1',  jenis:'Terlambat Masuk',     kat:'ringan',poin:10, tgl:'2026-03-11',guru:'Bu Siti', status:'baru',   lokasi:'Gerbang', deskripsi:'Terlambat karena macet.',notif:0},
    {id:9,inisial:'KP',color:'#8b5cf6',nama:'Kevin Prasetyo',   kelas:'XII RPL 1',jenis:'Perkelahian',         kat:'berat', poin:75, tgl:'2026-03-08',guru:'Bu Rina', status:'proses', lokasi:'Lapangan',deskripsi:'Perkelahian massal, masih diinvestigasi.',notif:1},
    {id:10,inisial:'AC',color:'#f59e0b',nama:'Ayu Cahyani',     kelas:'XI AKL 2',jenis:'Bolos Berulang',       kat:'sedang',poin:50, tgl:'2026-03-07',guru:'Pak Budi',status:'proses', lokasi:'—',       deskripsi:'Sudah 5 kali membolos dalam bulan ini.',notif:1},
    {id:11,inisial:'RM',color:'#ef4444',nama:'Rizal Maulana',   kelas:'X TSM 1', jenis:'Berkata Kasar',        kat:'sedang',poin:35, tgl:'2026-03-06',guru:'Bu Siti', status:'proses', lokasi:'Kelas',   deskripsi:'Berkata kasar kepada teman dan guru.',notif:0},
    {id:12,inisial:'TI',color:'#1a56db',nama:'Tari Indah',      kelas:'XI MM 2', jenis:'Seragam Tidak Sesuai', kat:'ringan',poin:10, tgl:'2026-03-05',guru:'Bu Rina', status:'proses', lokasi:'Kelas',   deskripsi:'Tidak pakai sepatu hitam dan kaos kaki putih.',notif:0},
    {id:13,inisial:'WN',color:'#ec4899',nama:'Wahyu Nugroho',   kelas:'XII TSM 2',jenis:'Membawa Senjata Tajam',kat:'berat',poin:100,tgl:'2026-03-04',guru:'Bu Rina', status:'proses', lokasi:'Tas',     deskripsi:'Kedapatan membawa pisau lipat di dalam tas.',notif:1},
    {id:14,inisial:'DS',color:'#10b981',nama:'Dina Safitri',    kelas:'XII TKJ 2',jenis:'Terlambat Masuk',     kat:'ringan',poin:10, tgl:'2026-03-01',guru:'Bu Rina', status:'diproses',lokasi:'Gerbang', deskripsi:'Sudah diberi pembinaan dan surat peringatan.',notif:1},
    {id:15,inisial:'FA',color:'#06b6d4',nama:'Fahmi Ardian',    kelas:'X RPL 1', jenis:'Membolos Pelajaran',   kat:'sedang',poin:30, tgl:'2026-02-28',guru:'Pak Budi',status:'diproses',lokasi:'—',       deskripsi:'Orang tua sudah dipanggil dan kasus ditutup.',notif:1},
    {id:16,inisial:'GM',color:'#8b5cf6',nama:'Gita Maharani',   kelas:'XI AP 2', jenis:'Seragam Tidak Sesuai', kat:'ringan',poin:10, tgl:'2026-02-27',guru:'Bu Siti', status:'diproses',lokasi:'Kelas',   deskripsi:'Sudah ganti seragam sesuai ketentuan.',notif:1},
    {id:17,inisial:'HK',color:'#f97316',nama:'Hendra Kusuma',   kelas:'XII MM 1',jenis:'Penggunaan HP di Kelas',kat:'sedang',poin:25, tgl:'2026-02-26',guru:'Bu Rina', status:'diproses',lokasi:'Kelas',   deskripsi:'HP dikonfiskasi dan dikembalikan ke orang tua.',notif:1},
];
...
function getByTab(tab){
    return tab==='semua'?[...allData]:allData.filter(d=>d.status===tab);
}
...
function renderTable(data){
    ...
    if(!data.length){
        tbody.innerHTML=`<tr><td colspan="9" style="text-align:center;padding:40px;color:var(--gray-400)">
            <i class="fa fa-inbox" style="font-size:28px;display:block;margin-bottom:10px;color:var(--gray-200)"></i>
            Tidak ada data pelanggaran</td></tr>`;
        document.getElementById('paginasiInfo').textContent='Tidak ada data';
        return;
    }
    document.getElementById('paginasiInfo').textContent=`Menampilkan 1–${data.length} dari ${data.length} data`;
    ...
    tbody.innerHTML=data.map((r,i)=>`
        <tr>
            ...
            <td>
                <div class="td-student">
                    <div class="td-avatar" style="background:${r.color}">${r.inisial}</div>
                    <div>
                        <div class="td-name">${r.nama}</div>
                        <div class="td-class">${r.kelas}</div>
                    </div>
                </div>
            </td>
            <td style="max-width:150px">${r.jenis}</td>
            <td><span class="badge ${r.kat}">${r.kat.charAt(0).toUpperCase()+r.kat.slice(1)}</span></td>
            <td><span class="poin-chip ${getPoinClass(r.poin)}">${r.poin} poin</span></td>
            <td style="color:var(--gray-400)">${fmtTgl(r.tgl)}</td>
            <td>${r.guru}</td>
            <td><span class="badge ${r.status}">${statusLabel[r.status]}</span></td>
            <td>
                <div class="action-btns">
                    <button class="btn-icon view" title="Lihat Detail"   onclick="openDetail(${r.id})"><i class="fa fa-eye"></i></button>
                    <button class="btn-icon edit" title="Edit"           onclick="openEdit(${r.id})"><i class="fa fa-pen"></i></button>
                    <button class="btn-icon ok"   title="Update Status"  onclick="openUpdateStatus(${r.id})"><i class="fa fa-rotate"></i></button>
                    <button class="btn-icon del"  title="Hapus"          onclick="openHapus(${r.id})"><i class="fa fa-trash"></i></button>
                </div>
            </td>
        </tr>
    `).join('');
...
function switchTab(tab,btn){
    ...
    document.getElementById('filterKls').value='';
    document.getElementById('filterGuru').value='';
    filteredData=getByTab(tab);
    renderTable(filteredData);
}

// ══ FILTER (submit ke server) ══
let filterTimer;
...
// ══ DETAIL ══
    currentDetailIdx = {id, nama, kelas, jenis, kat, poin, tgl, status, notif, lokasi, deskripsi}; {
    ...
    // Simpan untuk tombol Edit & Update Status di modal detail
    currentDetailIdx = {id, nama, kelas, jenis, kat, poin, tgl, status, notif, lokasi, deskripsi};
    ...
    const sl = {
        baru:     '<span class="badge baru">Baru</span>',
        proses:   '<span class="badge proses">Proses</span>',
        diproses: '<span class="badge diproses">Selesai</span>'
    };
